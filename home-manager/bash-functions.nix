{
  setPrompt = ''
    function set_prompt () {
      ...
      # Add VCS branch portion of the prompt, this adds ":branchname"
      if type -p jj > /dev/null 2>&1; then
        # jj is installed — use it for branch/bookmark info
        if jj root > /dev/null 2>&1; then
          JJ_BOOKMARK=''$(jj log --no-graph -r @ -T 'bookmarks.join(", ")' 2>/dev/null)
          if [ -z "$JJ_BOOKMARK" ]; then
            JJ_BOOKMARK=''$(jj log --no-graph -r @ -T 'change_id.shortest(8)' 2>/dev/null)
          fi
          PS1="''${PS1}:''${COLOR_GITBRANCH}''${JJ_BOOKMARK}''${COLOR_DIVIDER}"
        fi
      elif ! git_loc="''$(type -p "$git_command_name")" || [ -z "$git_loc" ]; then
      	# Git is installed
      ...
      PS1="[''${COLOR_USERNAME}\u''${COLOR_USERHOSTAT}@''${COLOR_HOSTNAME}\h''${COLOR_NONE}] ''${COLOR_DIVIDER}''${PS1}]''${COLOR_NONE} "
